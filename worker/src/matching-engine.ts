 ...
 *
 * VERBATIM COPY of src/services/matching.ts in the API. Kept byte-identical on purpose: the
 * worker scores to decide whether to open an application, and the API rescores at the write.
 * If the two drift, the worker starts opening applications the API then rejects — visible as
 * 409s in the run log. Change both together, or neither.
 *
